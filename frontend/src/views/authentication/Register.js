...
const teacherCode = process.env.REACT_APP_TEACHER_UNIQUE_CODE || '415110';
...
const userValidationSchema = yup.object({
  ...
  teacherUniqueCode: yup
    .string()
    .when('role', {
      is: 'teacher',
      then: (schema) =>
        schema
          .required('Teacher unique code is required')
          .oneOf([teacherCode], 'Invalid teacher unique code'),
...
const Register = () => {
  ...
  const handleSubmit = async ({ name, email, password, confirm_password, role, teacherUniqueCode, faceDescriptor }) => {
    ...
    if (role === 'teacher' && teacherUniqueCode !== teacherCode) {
      toast.error('Invalid teacher unique code');
      return;
    }
